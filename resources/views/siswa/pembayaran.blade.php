            ...
            <div class="w-64 bg-white rounded-xl shadow p-6 flex-shrink-0 relative -ml-9z-10">
                ...
                <div class="flex flex-col items-center">
                    <div class="w-20 h-20 bg-gray-300 rounded-full flex items-center justify-center text-xl font-bold text-gray-700">
                        {{ substr(Auth::user()->name, 0, 1) }}{{ substr(Auth::user()->name, 1, 1) }}
                    </div>
            ...
            <div class="flex-1">
                ...
                <div class="bg-white rounded-xl p-6 shadow text-center">
                    ...
                    <div class="grid grid-cols-2 gap-4 text-left max-w-lg mx-auto text-sm">
                        ...
                        <div>Tahun Ajaran</div> <div>2024/2025</div>
                        <div>NIS</div> <div>{{ Auth::user()->nis }}</div>
                        <div>Nama Siswa</div> <div>{{ Auth::user()->name }}</div>
                        <div>Kelas</div> <div>{{ Auth::user()->kelas }}</div>
                        <div>Jumlah Tagihan</div> <div>Rp. 180.000,00</div>

                        <div>Metode Pembayaran</div>
                        <div>
                            <select id="metode" class="border rounded px-3 py-1 w-32">
                                <option>Pilih</option>
                                <option>QRIS</option>
                            </select>
                        </div>
                    ...
                    <a href="{{ route('siswa.pembayaran') }}">
    <button class="mt-8 bg-green-600 text-white px-10 py-2 rounded-lg hover:bg-green-700">
        BAYAR
    </button>
</a>
