...
<body class="bg-gray-100 text-white flex flex-col min-h-screen overflow-hidden">
    ...
    <!-- Navbar -->
    <nav class="w-full bg-white text-black flex justify-between items-center px-8 py-4 shadow-md">
        <!-- Kiri: Logo + Nama Sekolah -->
        <div class="flex items-center space-x-3">
            <img src="{{ asset('images/logo.png') }}" alt="Logo SMK PGRI Gumelar" class="w-10 h-10 rounded-full">
            <span class="font-semibold text-sm md:text-base">SMK PGRI GUMELAR</span>
        </div>

        <!-- Kanan: Tombol -->
        <div class="flex space-x-4">
            <a href="{{ url('/siswa/daftar') }}" class="bg-[#0a1b3d] text-white px-5 py-1.5 rounded-full text-sm hover:bg-blue-900 transition">Daftar</a>
            <a href="{{ url('/siswa/masuk') }}" class="bg-[#0a1b3d] text-white px-5 py-1.5 rounded-full text-sm hover:bg-blue-900 transition">Masuk</a>
        </div>
    </nav>
